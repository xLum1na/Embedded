...
typedef struct servo_s *servo_handle_t;     ///< 不透明指针隐藏具体实现
...
typedef struct servo_ops_t {
     ...
     * 
     */
    void (*init)(servo_handle_t handle, const servo_config_t *cfg);
 ...
 * @brief  xxx舵机类构造函数
 */
void xxx_servo_create(servo_handle_t *handle);

/**
 * @brief  xxx舵机类析构函数
 */
void xxx_servo_delete(servo_handle_t *handle);
